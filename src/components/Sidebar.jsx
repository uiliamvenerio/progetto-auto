...
const QuickAccessItem = ({ image, name, lastSeen, amount, onClick }) => (
  <button
    onClick={onClick}
    className="flex items-center gap-4 w-full px-4 py-3 hover:bg-skyblue/30 dark:hover:bg-dark-hover transition-colors duration-200"
  >
    <div className="flex items-center gap-4 flex-1">
      <div
        className="bg-center bg-no-repeat bg-cover rounded-full h-10 w-10 border border-skyblue dark:border-gray-700"
...
export default function Sidebar({ isOpen, onClose, currentPage, onMenuItemClick }) {
  return (
    <aside className={clsx(
      'fixed inset-y-0 left-0 z-40 w-80 bg-white dark:bg-dark-card border-r border-skyblue dark:border-gray-800 lg:static transform transition-transform duration-300 ease-in-out',
      ...
      <div className="flex h-full flex-col">
        ...
        <div className="flex items-center justify-between p-4 lg:hidden">
          ...
          <button
            onClick={onClose}
            className="p-2 text-gray-500 hover:bg-skyblue/30 dark:hover:bg-dark-hover rounded-lg transition-colors"
          >
